...
export class RecordComponent {
  ...
  async uploadAudio() {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/v1/voice/record', {
        method: 'POST',
  ...
  async transcribeRecord(recordId: number) {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/v1/voice/transcribe?recordId=${recordId}`, {
        method: 'POST',
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
